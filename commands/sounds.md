...
```
python3 "${CLAUDE_PLUGIN_ROOT}/skills/sounds/scripts/sounds.py" $ARGUMENTS
```
...
translate it into the right subcommand first — see the `sounds` skill for the full mapping —
then run it. Never hand-edit `~/.claude/sounds.json`; always go through the CLI.
